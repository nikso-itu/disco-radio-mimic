...
static void iface_init(int argc, char *argv[])
{
	...
	int radio_addr = 150;		      // Default radio address
	...
	while ((opt = getopt(argc, argv, "i:p:a:")) != -1)
	{
		...
		case 'p':
			...
			break;
		case 'a':
			// Use the radio address
			radio_addr = atoi(optarg);
			break;
		default:
			fprintf(stderr, "Usage: %s [-i <interface>] [-p <port/device>] [-a <radio_address<´>]\n", argv[0]);
			exit(EXIT_FAILURE);
		}
	}
	...
	{
		int error = csp_can_socketcan_open_and_add_interface(can_device, "CAN", radio_addr, 100000, true, &iface);
		if (error != CSP_ERR_NONE)
		{
...
int main(int argc, char *argv[])
{
	...
	vmem_file_init(&vmem_image_buffer);
